...
%% X_numericas
% prueba con todas las variables, centradas
p = size(X_numericas, 2);
n = size(X_numericas, 1);
H = eye(n) - 1/n * ones(n); % matriz de centrado
X_numericas_0 = H*X_numericas;
[T1,D1,Y1,acum1,T2,D2,Y2,acum2]=comp2(X_numericas_0);

%% X_continuas
p = size(X_continuas, 2);
n = size(X_continuas, 1);
H = eye(n) - 1/n * ones(n); % matriz de centrado
X_continuas_0 = H*X_continuas;
[T1,D1,Y1,acum1,T2,D2,Y2,acum2]=comp2(X_continuas_0);

%% X_discretas
p = size(X_discretas, 2);
n = size(X_discretas, 1);
H = eye(n) - 1/n * ones(n); % matriz de centrado
X_discretas_0 = H*X_discretas;
[T1,D1,Y1,acum1,T2,D2,Y2,acum2]=comp2(X_discretas_0);


%% X_analisis
mean(X_analisis) % vemos que el vector de medias no es cero, por lo que la matriz no está centrada
% primero tenemos que centrar los datos
p = size(X_analisis, 2);
n = size(X_analisis, 1);
H = eye(n) - 1/n * ones(n); % matriz de centrado
X_analisis_0 = H*X_analisis;
mean(X_analisis_0) % la media es cero, por lo tanto la matriz está centrada

[T1,D1,Y1,acum1,T2,D2,Y2,acum2]=comp2(X_analisis_0); % supuestamente deberíamos escoger
...
%% INTERPRETACIÓN DE PCA: en jupyter
% A la hora de interpretar debemos tener en cuenta que estamos trabajando
...
%[X_continuas_transformada, lambda_2, lambda_1, p_valores] = transformar_boxcox(X_continuas, nombres_variables_numericas, false) % para que no se impriman los histogramas
lambda_2_analisis = lambda_2(setdiff(1:size(X_continuas_transformada, 2), [2,3]))'
lambda_1_analisis = lambda_1(setdiff(1:size(X_continuas_transformada, 2), [2,3]))
